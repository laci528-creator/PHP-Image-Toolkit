...
function Format_konvert( string $pfad, string $neupfad, string $outputformat, int $q):bool {
...
if (!file_exists($pfad)) {
    ...
    if ($info === false || !isset($info['mime'])) {
        ...
        $ok = false;
            ...
            switch($outputformat) {
                case "jpeg":
                    $ok = imagejpeg($image,$neupfad,$q);
                    break;
                case "png":
                    $pngQuality = (int) round((100 - $q) / 100 * 9); 
                    $ok = imagepng($image,$neupfad, $pngQuality);
                    break;
                case "webp":
                    $ok = imagewebp($image,$neupfad,$q);
                    break;
                case "avif":
                    $ok = imageavif($image,$neupfad,$q);
                    break;
